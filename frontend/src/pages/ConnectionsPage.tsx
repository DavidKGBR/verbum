...
import ArcDiagramPage from "./ArcDiagramPage";
...
export default function ConnectionsPage() {
  ...
  const active: Tab = isTab(raw) ? raw : "arc";
      ...
      <div role="tablist" aria-label={t("nav.connections")} className="flex gap-1 border-b border-white/10">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {t(`connections.tab.${tab}`)}
            </button>
          );
        })}
      </div>
      <div role="tabpanel">
        {active === "arc" && <ArcDiagramPage />}
        {active === "graph" && <SemanticExplorerPage />}
        {active === "citations" && <IntertextualityPage />}
      </div>
    </div>
  );
}
