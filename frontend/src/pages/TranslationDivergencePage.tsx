...
export default function TranslationDivergencePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="page-title text-2xl">Translation Divergence</h1>
        <p className="text-sm opacity-60 mt-1">
          See how different translations render the same Hebrew/Greek word
          side by side. Reveals theological and stylistic choices.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-4 mb-6 items-end">
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-xs opacity-60">Strong's ID</span>
      ...
      {loading && <LoadingSpinner text="Loading comparisons..." />}
        ...
        <>
          ...
          <div className="rounded-lg border bg-white p-4 mb-6">
            <div className="flex items-baseline gap-3 flex-wrap">
              ...
              <span className="text-xs opacity-50 ml-auto">
                {data.total_verses} total verses · showing {data.verses.length}
              </span>
            </div>
          </div>
          ...
          {data.verses.length === 0 && (
            <p className="text-sm opacity-50 italic mt-4">
              No verses found for {data.strongs_id} in the selected translations.
            </p>
          )}
        </>
      )}
      ...
      {!data && !loading && (
        <div className="rounded-lg border border-dashed border-[var(--color-gold-dark)]/30 p-8 text-center">
          <p className="opacity-60 mb-3">
            Enter a Strong's ID and click Compare to see how translations differ.
          </p>
